...
param(
  ...
  [string]$BundleName = "wonder-estimate-opencode-assets.zip",
  [switch]$Json,
  [switch]$Force
)
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("wonder-estimate-bundle-" + [System.Guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
try {
  $staging = Join-Path $tempDir "estimate"
