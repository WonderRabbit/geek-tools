...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("wonder-estimate-verify-" + [System.Guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $tempDir -Force | Out-Null
try {
  [System.IO.Compression.ZipFile]::ExtractToDirectory($bundleResolved, $tempDir)
  $extractRoot = Join-Path $tempDir "estimate"
  if (-not (Test-Path -LiteralPath $extractRoot)) {
    $extractRoot = $tempDir
  }
  $doctor = Join-Path $extractRoot ".opencode/estimate/scripts/doctor.ps1"
...
if ($Json) {
  ...
  "VERIFY_BUNDLE_$($overall.ToUpperInvariant())"
  foreach ($check in $checks) {
