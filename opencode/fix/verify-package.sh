...
IMPL="$ROOT/impl"
PATCH="$IMPL/patches/opencode-write-json-repair.patch"
...
verify_installer_static() {
  require_text "$IMPL/install-opencode-write-json-repair.ps1" 'CmdletBinding\(SupportsShouldProcess'
  require_text "$IMPL/install-opencode-write-json-repair.ps1" 'Apply-UnifiedPatch'
  require_text "$IMPL/install-opencode-write-json-repair.ps1" 'New-Backup'
  require_text "$IMPL/install-opencode-write-json-repair.ps1" 'Restore-LatestBackup'
  require_text "$IMPL/install-opencode-write-json-repair.ps1" 'delete`t'
  require_text "$IMPL/install-opencode-write-json-repair.ps1" 'Remove-Item -LiteralPath'
  require_text "$IMPL/install-opencode-write-json-repair.ps1" 'Assert-WindowsTarget'
  require_text "$IMPL/install-opencode-write-json-repair.ps1" 'ForceUnsupportedOS'
  require_text "$IMPL/install-opencode-write-json-repair.ps1" 'bun.+typecheck|typecheck'
  echo "INSTALLER_STATIC_OK"
}

verify_docs_static() {
  require_text "$IMPL/INSTALLATION.md" 'Windows 10'
  require_text "$IMPL/INSTALLATION.md" 'PowerShell 7\.6'
  require_text "$IMPL/INSTALLATION.md" 'install-opencode-write-json-repair\.ps1'
  require_text "$IMPL/INSTALLATION.md" 'Rollback|롤백'
  require_text "$IMPL/INSTALLATION.md" 'bun run --cwd packages/opencode typecheck'
  echo "DOCS_STATIC_OK"
}
...
verify_patch_apply() {
  if [[ ! -d "$TARGET/.git" ]]; then
    echo "SKIP_PATCH_APPLY target missing: $TARGET"
    return 0
  fi
  local tmp
